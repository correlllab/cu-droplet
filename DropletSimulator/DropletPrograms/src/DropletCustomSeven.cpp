...
void DropletCustomSeven::handle_start_broadcast(){
...
void DropletCustomSeven::check_ready_to_move(){
...
void DropletCustomSeven::awaiting_confirmation(){
	while(check_for_new_messages()){
...
void DropletCustomSeven::adj_spots_to_fill(){
	if(rand_byte()<8){
		char msg[2];
		msg[0] = IN_ASSEMBLY_INDICATOR_BYTE;
		msg[1] = my_spots_to_fill;
...
void DropletCustomSeven::print_msg(){
	//fprintf(file_handle,"\t");
...
void DropletCustomSeven::decide_if_should_move(){
	if(rand_byte()<64){
	...
	while(check_for_new_messages()){
		...
		//fflush(file_handle);
		if(((global_rx_buffer.data_len-2 /*JOHN: data_len has a bug?*/)==sizeof(favTgtMsg7))){
			print_msg();
			favTgtMsg7* msg = (favTgtMsg7*)global_rx_buffer.buf;
			...
			//fflush(file_handle);
			std::map<droplet_id_type, recruitingRobot7*>::iterator iter;
...
void DropletCustomSeven::waiting_for_message(){
	...
	if((!check_timer(DECIDING_DELAY_TIMER)) || recruiting_robots.empty()){
		while(check_for_new_messages()){
			...
			if(((global_rx_buffer.data_len-2 /*JOHN: data_len has a bug?*/)==2) & (global_rx_buffer.buf[0] == IN_ASSEMBLY_INDICATOR_BYTE)){ //the talking bot is in the assembly. We want to be in the assembly. Lets listen! 
				...
				fflush(file_handle);
				recruitingRobot7* target = new recruitingRobot7;
...
void DropletCustomSeven::broadcast_favorite_target(){
	favTgtMsg7* msg = new favTgtMsg7;
	msg->type = NOT_IN_ASSEMBLY_INDICATOR_BYTE;
	msg->id = closestID;
	msg->dir = closestDir;
	msg->dist = closestDist;
	ir_broadcast((char*)msg, sizeof(favTgtMsg7));
}

float DropletCustomSeven::get_distance(float rA, float thetaA, float rB, float thetaB){
...
uint8_t DropletCustomSeven::get_spots_from_pos(uint8_t dir_pos){
	switch(dir_pos){
...
move_direction DropletCustomSeven::get_best_move_dir(float theta){
...
void DropletCustomSeven::calculate_distance_to_target_positions(){
	closestDist = std::numeric_limits<float>::infinity();
	closestDir = 0;
	closestID = 0;
	for(std::map<droplet_id_type, recruitingRobot7*>::iterator iter=recruiting_robots.begin() ; iter!=recruiting_robots.end(); iter++){
...
float DropletCustomSeven::getAngleFromDirMask(uint8_t dir_mask){
	float angle=0;
	switch(dir_mask){
...
void DropletCustomSeven::get_relative_neighbor_position(uint8_t dir_mask, float neighbor_r, float neighbor_theta, float neighbor_phi, float* target_r, float* target_theta){
...
void DropletCustomSeven::add_polar_vec(float r1, float th1, float r2, float th2, float* rs, float* ths){
...
void DropletCustomSeven::remove_dir_from_spots_map(uint8_t dir, droplet_id_type id){
...
float DropletCustomSeven::deg2rad(float deg){
	return (float)((deg*((float)M_PI))/180.0);
}

float DropletCustomSeven::rad2deg(float rad){
