...
void DropletCustomNine::handle_start_broadcast(){	
...
void DropletCustomNine::handle_move_to_center(){
	if(is_moving()){
		//do nothing.
	}else{
		float dx=0, dy=0, phi;
...
void DropletCustomNine::decide_if_should_move(){
...
void DropletCustomNine::awaiting_confirmation(){
...
void DropletCustomNine::adj_spots_to_fill(){
	while(check_for_new_messages()){
...
void DropletCustomNine::handle_move_to_spot(){
	if(is_moving()){
...
void DropletCustomNine::handle_adjusting_phi(){
	if(is_moving()){
		//Do nothing.
	}else{
		float r, theta, phi;
...
void DropletCustomNine::waiting_for_message(){
...
void DropletCustomNine::reset_before_waiting_for_msgs(){
	set_timer(RANDOM_WALK_DELAY_MS, RANDOM_WALK_TIMER);
	move_target = NULL;
	move_target_dir = 0;
	closestDir=0;
...
void DropletCustomNine::handle_rotate_to_straight(float theta){
...
void DropletCustomNine::call_for_neighbors(){
	//fprintf(file_handle, "\tCalling for neighbors.\n");
	//fflush(file_handle);
	char msg[2];
	msg[0] = IN_ASSEMBLY_INDICATOR_BYTE;
...
bool DropletCustomNine::check_if_stuck(float delta, float last_delta){
...
void DropletCustomNine::maintain_position(droplet_id_type bot, uint8_t dir){
	if(is_moving()){
		//already moving, great.
	}else{
...
void DropletCustomNine::broadcast_favorite_target(){
...
void DropletCustomNine::broadcast_claim_msg(droplet_id_type parent, uint8_t dir){
...
float DropletCustomNine::get_distance(float rA, float thetaA, float rB, float thetaB){
...
move_direction DropletCustomNine::get_best_move_dir(float theta){
...
void DropletCustomNine::calculate_distance_to_target_positions(){
...
float DropletCustomNine::getAngleFromDirMask(uint8_t dir_mask){
	float angle=0;
	switch(dir_mask){
...
void DropletCustomNine::get_relative_neighbor_position(uint8_t dir_mask, float neighbor_r, float neighbor_theta, float neighbor_phi, float* target_r, float* target_theta){
...
void DropletCustomNine::add_polar_vec(float r1, float th1, float r2, float th2, float* rs, float* ths){
...
float DropletCustomNine::sub_polar_vec_mag(float r1, float th1, float r2, float th2){
	float x1=r1*cos(deg2rad(th1));
	float y1=r1*sin(deg2rad(th1));
	float x2=r2*cos(deg2rad(th2));
	float y2=r2*sin(deg2rad(th2));
...
void DropletCustomNine::remove_dir_from_spots_map(uint8_t dir, droplet_id_type id){
...
float DropletCustomNine::deg2rad(float deg){
	return (float)((deg*((float)M_PI))/180.0);
}

float DropletCustomNine::rad2deg(float rad){
	return (float)((rad*180.0)/((float)M_PI));
}

float DropletCustomNine::quick_and_dirty_mod(float theta){
	while(theta>180.0){
		theta=theta-360.0;
	}
	while(theta<-180.0){
