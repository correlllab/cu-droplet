		...
		while(incomingMsgHead != NULL){
			...
			while(tmp->next !=NULL){
				...
				if(crcMatches && closeTimes){
					...
					myFree(deleteMe);
					memoryConsumedByIncomingMsgBuffer -= (sizeof(IncMsgNode) + msgStruct.length);
					numWaitingMsgs--;
				}else{
					tmp = tmp->next;
				}
			...
			myFree(node);
			memoryConsumedByIncomingMsgBuffer -= (sizeof(IncMsgNode) + msgStruct.length);
			//printf("\tmemoryConsumedByMsgBuffer=%u and  Data length = %hu\r\n",memoryConsumedByIncomingMsgBuffer, msgStruct.length);	//RIYA			
