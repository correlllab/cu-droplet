...
{		
	...
	if (ir_rx[dir].ir_status & IR_RX_STATUS_BUSY_bm)	return 2;		// Error 2: receiver busy *probably ditch*
	...
	/* 
	 * Include sender ID in crc calculation. If using 16-bit IDs, truncate to 8-bit for this calculation.
	 */
	...
	memcpy(ir_rxtx[dir].buf, &(tx_msg_header[dir]), HEADER_LEN);//replace
	...
	channel[dir]->CTRLA |= USART_DREINTLVL_MED_gc;		/* Enable medium level DRE interrupt to begin transmission*/	
...
void ir_receive(uint8_t dir)
{
	...
	{
		case HEADER_POS_MSG_LENGTH:		ir_rxtx[dir].data_len	  = in_byte; break;
		...
		case HEADER_POS_SENDER_ID_LOW:  ir_rxtx[dir].sender_ID |= (((uint16_t)in_byte)<<8); break;
	...
	if(ir_rxtx[dir].curr_pos==(ir_rxtx[dir].data_len+HEADER_LEN)) //Message done?
	{
		if(/*check header if targeted at me or not targeted*/)
			if(/*check header if targeted*/) 
			else //handle normale message
	}
...
{	
	...
	if(ir_rxtx[dir].curr_pos >= ir_rxtx[dir].data_len+HEADER_LEN)
	{
		channel[dir]->CTRLA &= ~USART_DREINTLVL_gm;
		return;	// this message is over, do not send any more
	}
	
	uint8_t tx_data = ir_rxtx[dir].buf[ir_rxtx[dir].curr_pos];
	channel[dir]->DATA = tx_data;		// Data heading out.
...
void ir_transmit_complete(uint8_t dir)
{
	...
	for (uint8_t i = 0; i < 6; i++) channel[i]->CTRLB |= USART_RXEN_bm;	// this enables receive on the USART
}
