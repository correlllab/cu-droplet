...
1. Each droplet has a random RGB value and show the color
2. Each droplet shares its RGB value with its neighbors at each round
3. Define the graph, compute the degree of each droplet by counting the number of its neighbors
4. Compute the histogram it gets from neighbors
5. Show a different color with the information got


# Technical issues:
1. Define the graph using the droplets’ positions/ just find out who are its neighbors? How?
