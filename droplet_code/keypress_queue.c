...
static inline uint8_t sameEvent(KeypressEvent* evtA, KeypressEvent* evtB){
	return ((evtA->src == evtB->src) && (evtA->key == evtB->key) && ((evtA->time - evtB->time) < MIN_MULTIPRESS_DELAY));
}

static inline uint8_t isEventInit(KeypressEvent* evt){
	return !((evt->key == KEYBOARD_UNKNOWN) && (evt->src == 0xFFFF));
}
...
void queueInit(){
	for(uint8_t i=0;i<NUM_LOGGED_EVENTS;i++){
		...
		keypressLog[i].key = KEYBOARD_UNKNOWN;
	}
}
