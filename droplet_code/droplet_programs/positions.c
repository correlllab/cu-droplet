...
//void loop(){
	...
	//if(loopID!=lastLoop){
		//if(loopID==mySlot){
			...
			//if((rand_byte()&0xFF)){ //Small chance to simply not do a broadcast. (1/16, currently)
				//broadcast_rnb_data();
			//}
			//delay_ms(8+(rand_byte()>>3));
			//sendNeighbMsg();
...
//float calculateFactor(float dist, float conf, uint8_t dir, uint16_t id, float* dfP, float* nfP){
	//float factor, distFactor, confFactor, neighbFactor;
	//
	...
	////BAYES_DEBUG_PRINT("%7.3f\t(%7.3f, %7.3f, %7.3f)",  factor, distFactor, neighbFactor, confFactor);
	//(*dfP) =  distFactor;
	//(*nfP) = neighbFactor;
	//return factor;
//}
		...
		//for(uint8_t k=0;k<NUM_TRACKED_BAYESIAN;k++){
			//if(candidates[k].id!=0){
				//if(candidates[k].id==nearBots[j].id){
					...
					//if(dists[j]>GRID_DIMENSION) numNonadjacent++;
