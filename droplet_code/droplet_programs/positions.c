...
//
	//if(!(isnan(myX) || isnan(myY))){
		//float otherX = myX + neighbPos[dir][0];
		//float otherY = myY + neighbPos[dir][1];
		//if(otherX<MIN_DIM || otherX > X_MAX_DIM || otherY<MIN_DIM || otherY>Y_MAX_DIM){
			//factor = factor/4;
		//}
	//}
	//
	//if(noRecipPenalty[dir]<1.0){
		//printf("\tNo recip for %hu: %f\r\n",dir, noRecipPenalty[dir]);
	//}
		...
		//for(uint8_t k=0;k<NUM_TRACKED_BAYESIAN;k++){
			//if(candidates[k].id!=0){
				//if(candidates[k].id==nearBots[i].id){
					...
					//BAYES_DEBUG_PRINT("\t%04X: %7.3f\t(%4d -> %7.3f, %7.3f, %7.3f) -> [ f] <-\r\n", nearBots[i].id, factor, (int16_t)(10*dists[i]), distFactor, neighbFactor, nearBots[i].conf);
