...
void DropletCustomEight::decide_if_should_move(){
	...
	while(check_for_new_messages()){
		if((global_rx_buffer.data_len==sizeof(favTgtMsg))){
...
void DropletCustomEight::awaiting_confirmation(){
	...
	claimMsg* msg;
	while(check_for_new_messages()){
		if((global_rx_buffer.buf[0]==IN_ASSEMBLY_INDICATOR_BYTE)&&(global_rx_buffer.data_len==6)){
...
void DropletCustomEight::adj_spots_to_fill(){
	while(check_for_new_messages()){
		if((global_rx_buffer.buf[0]==NOT_IN_ASSEMBLY_INDICATOR_BYTE)&&(global_rx_buffer.data_len==4)){
			droplet_id_type ack_tgt = *((droplet_id_type*)(global_rx_buffer.buf+1));
...
void DropletCustomEight::handle_move_to_spot(){
	...
	while(check_for_new_messages()){
		if(global_rx_buffer.data_len==sizeof(claimMsg)){
		...
		}else if(global_rx_buffer.data_len==2){
...
void DropletCustomEight::handle_pre_assembly(){
	...
	while(check_for_new_messages()){
		if((((claimMsg*)global_rx_buffer.buf)->type==CLAIM_MSG_TYPE)||(global_rx_buffer.buf[0] == IN_ASSEMBLY_INDICATOR_BYTE)){
			...
			if((global_rx_buffer.data_len==2) && (global_rx_buffer.buf[0] == IN_ASSEMBLY_INDICATOR_BYTE)){
...
void DropletCustomEight::waiting_for_message(){
	if((get_32bit_time()-deciding_delay_start)<=DELAY_BEFORE_DECIDING_MS || recruiting_robots.empty()){
		while(check_for_new_messages()){
			if((global_rx_buffer.data_len==2) && (global_rx_buffer.buf[0] == IN_ASSEMBLY_INDICATOR_BYTE)){ //the talking bot is in the assembly. We want to be in the assembly. Lets listen! 
				add_recruiting_robot(global_rx_buffer.sender_ID, global_rx_buffer.buf[1]);
			}
			global_rx_buffer.read = 1;
		}
