...
static inline void irOn(uint8_t dir){
	// low signal on TX pin (remember: these pins were inverted during init)
	switch(dir){
		case 0: PORTC.OUT &= ~PIN3_bm; break;
		case 1: PORTC.OUT &= ~PIN7_bm; break;
		case 2: PORTD.OUT &= ~PIN3_bm; break;
		case 3: PORTE.OUT &= ~PIN3_bm; break;
		case 4: PORTE.OUT &= ~PIN7_bm; break;
		case 5: PORTF.OUT &= ~PIN3_bm; break;
	}
}

static inline void irOff(uint8_t dir){
...
static inline void sendBit(uint8_t dir, uint8_t bit){
	irOn(dir);
	busy_delay_us(562.5);
	irOff(dir);
	if(bit){	//Logical '1'
		busy_delay_us(1687.5);
	}else{				//Logical '0'
		busy_delay_us(562.5);
	}
}

static inline void sendByte(uint8_t dir, uint8_t byte){
	sendBit(dir, byte&0x01);
	sendBit(dir, byte&0x02);
	sendBit(dir, byte&0x04);
	sendBit(dir, byte&0x08);
	sendBit(dir, byte&0x10);
	sendBit(dir, byte&0x20);
	sendBit(dir, byte&0x40);
	sendBit(dir, byte&0x80);
}
